...
const _getFloorplan = async (orgId: string, floorplanSlug: string) => {
  ...
  const { data, error } = await supabase
    .from("floorplans")
    .select("id, name, slug, community, price_sheet_label, contract_locked_ids, sync_pairs, is_active, cover_image_path, loom_url, calendly_url, is_prospect_demo, prospect_insights, hero_headline, hero_body, preset_variations")
    .eq("org_id", orgId)
...
const _getFloorplansForOrg = async (orgId: string) => {
  ...
  const { data, error } = await supabase
    .from("floorplans")
    .select("id, name, slug, is_active, cover_image_path, is_prospect_demo")
    .eq("org_id", orgId)
    .neq("is_prospect_demo", true)
    .order("name");
    ...
    supabase
      ...
      .select(`
        ...
        subcategories (
          ...
          options ( id, slug, name, price, prompt_descriptor, dimensions, swatch_url, swatch_color, nudge, sort_order, floorplan_ids, generation_rules, is_default, needs_isolation, linked_to_subcategory )
        )
      `)
      .eq("org_id", orgId)
      .order("sort_order"),
...
// ---------- Multi-pass pipeline: pass_cache queries ----------

/**
 * Look up a cached intermediate image for a specific pass.
 * Returns the image path if found, null if miss.
 */
export async function findPassCacheHit(
  stepPhotoId: string,
  passName: string,
  passHash: string,
  upstreamHash: string,
...
/**
 * Store a pass intermediate in the cache. Uses ON CONFLICT DO NOTHING
 * so concurrent writes for the same pass/hash are safe.
 */
export async function upsertPassCache(params: {
  stepPhotoId: string;
  orgId: string;
  passName: string;
  passHash: string;
  upstreamHash: string;
  model: string;
  passSelectionsJson: Record<string, string>;
  imagePath: string;
}): Promise<void> {
  const supabase = getServiceClient();
  const { error } = await supabase
    .from("pass_cache")
    .upsert({
      step_photo_id: params.stepPhotoId,
      org_id: params.orgId,
      pass_name: params.passName,
      pass_hash: params.passHash,
      upstream_hash: params.upstreamHash,
      model: params.model,
      pass_selections_json: params.passSelectionsJson,
      image_path: params.imagePath,
    }, { onConflict: "step_photo_id,pass_name,pass_hash,upstream_hash", ignoreDuplicates: true });

  if (error) {
    console.warn("[pass_cache] Upsert failed:", error.message);
  }
}
