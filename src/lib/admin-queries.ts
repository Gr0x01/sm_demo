  ...
  const { data: cats, error } = await supabase
    ...
    .select(`
      ...
      subcategories (
        ...
        options ( id, slug, subcategory_id, org_id, name, price, description, prompt_descriptor, dimensions, swatch_url, swatch_color, nudge, is_default, sort_order, floorplan_ids, generation_rules )
      )
    `)
    .eq("org_id", orgId)
    .order("sort_order");
  ...
  const { data: steps, error } = await supabase
    ...
    .select(`
      ...
      step_photos (
        ...
        checked_at, spatial_hint, photo_baseline, subcategory_ids, prompt_prose, created_at
      )
    `)
    .eq("floorplan_id", floorplanId)
    .eq("org_id", orgId)
