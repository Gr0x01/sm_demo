...
| Nest Bathroom | ✓ done 2026-04-14 original; **re-tuned 2026-04-18** under Flex+Klein-only pipeline — 7 prose clauses rewritten (towel-bar/TP-holder fix, floor-tile geometry anchor, `apply` verb on faucet after "widespread" was dropped as swatch-authority violation, round/wall-mounted mirror, generic cab-hw pulls+knobs), Driftwood render_mode flipped hex_paint → hex_stain, floor-tile scoped edits now strip the word "hex" from the anchor, bath-hardware now gets hex anchor via `isCabinetHardwareSubcategory` narrowing |
...
**Shipped 2026-04-18 (Nest bathroom Flex validation + D102 hex-word rule):**

19. **D102 hex-anchor narrowing for floor-tile subcategories.** Lab on Nest bathroom uncovered that the literal word `hex` in the D102 injection (`image N at hex #XXXXXX`) activates Flex's hex-mosaic-tile prior on floor-tile slugs — Calacatta 12x24, Omega Grey 13x13, Omega Silver 13x13, Onyx 12x24 Matte all rendered as hex-mosaic in Flex-scoped regardless of actual swatch layout. Dropping the word `hex` (now `image N at #XXXXXX`) fixed all four tiles. First attempt was global (both full-gen and scoped sites); RB called it out as an over-scoped untested global change. Narrowed to `isFloorTileSubcategory(subId)` helper — only floor-tile slugs strip the word `hex` from the anchor. Counters, backsplashes, fireplace tile surrounds, showers stay on the full `at hex #` format that's known to work in production. Code: `src/lib/step-config.ts` (new helper) + `src/lib/generate.ts` (2 hex anchor substitution sites branch on it). 2 new tests cover both branches (floor-tile emits `at #`, countertop keeps `at hex #`). 284 tests pass.

20. **Floor-tile scoped containment via geometry-anchored clause.** Nest bath clause shipped 2026-04-17 as `change the main bathroom floor tile to match {image}` — the word `tile` alongside the swatch's hex anchor was pulling shower wall tile (same visual class) along with the floor swap on Omega Grey. New clause: `change the horizontal bathroom floor under the tub and vanity to match {image}`. `under the tub and vanity` names two fixed landmarks outside the shower, implicitly excluding the shower enclosure without naming it (naming shower reactivates the class; see `feedback_dont_name_what_you_dont_want_changed.md`). Validated clean containment on both Omega Grey (was bleeding) and Calacatta (regression check). Applied to DB.

21. **`isCabinetHardwareSubcategory` narrowing — bath-hardware now gets hex anchor.** D103 metallic hex-skip was defined against `isHardwareSubcategory` (substring `"hardware"`) which swept up `bath-hardware` (wall-mounted towel bars, TP holders) alongside cabinet pulls. Wall accessories behave differently from cabinet pulls — the D103 bronze-distortion rule was only ever validated on cabinet hardware, and bath-hardware was failing silver-finish swaps because the swatch alone wasn't strong enough to overcome a matte-black source prior. Tiburon Satin Nickel rendered clean silver in both scoped and bundled once the hex anchor was active. New helper `isCabinetHardwareSubcategory(subId) = subId.includes("cabinet-hardware")` matches only `kitchen-cabinet-hardware` + `bathroom-cabinet-hardware`. Three hex-skip sites in `generate.ts` switched over.

22. **Faucet swatch-authority rule — spec descriptors block silver swaps.** Original bath-faucets clause was `replace both widespread vanity sink faucets with {image}`. `widespread` describes the installed spec (4"+ spread vs centerset 4") and is a swatch-authority violation: it tells Flex what the current faucet is, not what the swatch depicts. Against a Nano Banana matte-black source, that anchored the current finish and blocked the Holliston Brushed Nickel swatch from overriding the color. Dropping `widespread` made silver swap cleanly on all 3 verbs (change/apply/replace) in scoped isolation. Shipped with `apply` verb — bundled context produced more reflective/metallic cab-hardware sheen than `change` in side-by-side. Final clause: `apply {image} to both vanity sink faucets`. Same principle ("don't describe the installed fixture") extends to any spec/format descriptor.

23. **Driftwood render_mode fix** (`primary-bath-cab-driftwood`) — was `hex_paint` with hex `#B09A7E`, kitchen twin `kitchen-cab-color-driftwood` was correctly `hex_stain` with same hex since day one. Bath option was mis-tagged at seed time. Flipped to `hex_stain` via scoped UPDATE (Demo org only). Prose dispatcher now routes Driftwood bath cab to the stain clause (`stain every vanity door and drawer with wood grain matching {image}`) via the per-material `{ paint, stain }` object shipped in PR #5/D104.

24. **Clause testing methodology codified.** RB formalized the verb-sweep + descriptor-toggle pattern he uses when testing clause wording — sweep `change`/`replace`/`apply` across the 3 non-material verbs, toggle any suspect descriptor on/off, test in the same path (scoped vs bundled) as the production failure. Captured as memory for future lab work instead of asking each time. Faucet test above followed this pattern: 3 verbs × with-widespread baseline = 4 variants.

25. **Memory bank hygiene.** Rules added/updated for future work: scope runtime changes before editing (not after), never name surfaces you want preserved (activates the class), test with visible contrast (no black-on-black), no default-only baseline variants, open the review HTML after a run (not the config during authoring).
