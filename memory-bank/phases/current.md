...
**Prospect demo + prompt fixes (2026-04-07):**
- **Alexander Scott Homes** demo built: `withfin.ch/for/alexander-scott` (Langston kitchen at Swann's Bridge, Auburn AL). 5 subcategories including island cabinet two-tone (table-style legs). Hero: "I used your kitchen." Body references Langston + Swann's Bridge. 3 presets: Standard / Mid-Range (admiral blue island) / Premium (fog cabs, onyx island, herringbone). Target: Cole Jolly (President, accepted LinkedIn connection). Research brief at `memory-bank/prospects/alexander-scott.md`. Config at `scripts/prospect-configs/alexander-scott.json`.
- **Background**: Founded 2023 by Warren Jolly (ran The Providence Group in Atlanta 20+ yrs, 5,000 sqft Design Studio) and son Cole. 3 communities, 1,500 lots in pipeline, $320K-$520K, zero viz tools. Auburn AL (~1.5hrs from Montgomery).
- **Prompt fix — "natural sunlight"**: Full gen changed from "natural lighting" to "natural sunlight" (lamps are "natural" too — dark surfaces were darkening the room). Scoped edits now append "Preserve natural sunlight." Both changes in `generate.ts`.
- **Subagent guardrails**: bfl-prompt-engineer updated — must start from proven /try patterns, forbidden words list (island, current colors, "back wall", invisible appliances), keep hints under 20 words. prospect-demo-builder updated — must delegate prompts to bfl-prompt-engineer, copy to copywriter/growth-hacker, must query DB for option slugs (never guess).
- **Key lesson — simple hints work better**: Every attempt to add descriptive language (alcove descriptions, "flanking the chimney hood on both sides", "along the back wall") made generation WORSE. The proven /try patterns (5-15 words) consistently outperform longer hints. Extra words give Flux more to misinterpret.
- **Key lesson — don't adapt subagent output**: Use bfl-prompt-engineer output verbatim. Adapting, combining, or "improving" its text reintroduces the problems it was trying to avoid.
